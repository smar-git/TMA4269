...
- Log into NTNU JupyterHub [https://jupyter.ntnu.no/](https://jupyter.ntnu.no/)
...
Remember: $$y_i = \underbrace{\beta_0 + \beta_1\ x_i}_{\text{Sistematic part}} + \underbrace{\epsilon_i}_{\text{Random part/ error}}, \qquad \epsilon_i\sim\mathcal{N}(0,\sigma^2)$$
...
-   You could consider instead to use *log(distance)*, why would that make sense? Does the effect of distance appear to be the same everywhere? For example, would increasing distance from 1 → 2 km have the same effect as increasing it from 8 → 9 km?
...
-   What is the difference between these two types of intervals?
